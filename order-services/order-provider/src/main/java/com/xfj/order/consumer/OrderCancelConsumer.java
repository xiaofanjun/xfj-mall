package com.xfj.order.consumer;

import com.xfj.commons.config.RabbitMqConfig;
...
public class OrderCancelConsumer {
    ...
    try {
		...
		if(order1.getStatus()==0){
			...
			List<String> itemIds=list.stream().map(OrderItem::getItemId).sorted().collect(Collectors.toList());
			//锁 itemIds
